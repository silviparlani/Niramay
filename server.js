...
app.post('/checkData2', (req, res) => {
  ...
  const queryCustomers = `SELECT COUNT(*) AS count FROM customers WHERE anganwadi_no = ? AND child_name = ?`;
  db.query(queryCustomers, [anganwadiNo, childsName], (err, results) => {
    if (err) {
...
app.post('/visits', (req, res) => {
  try {
    const { anganwadiNo, childName, visitDate, haemoglobin, totalNoOfJars, grade, weight, height, muac, difference, observations, iron, calcium, multivitamin, protein } = req.body;
    console.log('Request Body:',req.body);
    const sql = 'INSERT INTO Visits (anganwadiNo, childName, visitDate,haemoglobin,totalNoOfJars,grade, weight, height, muac, difference, observations, iron, calcium, multivitamin, protein) VALUES (?, ?, ?, ?, ?, ?, ?,?,?,?,?, ?,?,?,?)';
    db.query(sql, [anganwadiNo, childName, visitDate, haemoglobin, totalNoOfJars, grade, weight, height, muac, difference, observations, iron, calcium, multivitamin, protein], (err, result) => {
      if (err) {
